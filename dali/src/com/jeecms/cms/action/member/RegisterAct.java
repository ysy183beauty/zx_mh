...
public class RegisterAct {
	...
	public String submit(String username, String email, String loginPassword,
		...
		WebErrors errors = validateSubmit(username, email, loginPassword, type, captcha,
				site, request, response);
		boolean disabled=false;
		if(config.getMemberConfig().isCheckOn()){
			disabled=true;
		}
		if (errors.hasErrors()) {//修改为返回注册页面
            model.addAttribute("registerUserName",username);
            model.addAttribute("registerEmail",email);
            model.addAttribute("registerPassWord",loginPassword);
            errors.toModel(model);
            FrontUtils.frontData(request, model, site);
            FrontUtils.frontPageData(request, model);
            String tpl = FrontUtils.getTplPath(request, site.getSolutionPath(), TPLDIR_ALONE, REGISTER_RESULT);
		...
		if (config.getEmailValidate()) {
			...
			String tpl = FrontUtils.getTplPath(request, site.getSolutionPath(),
                    TPLDIR_ALONE, REGISTER_ACTIVE_SUCCESS);
			return tpl;
	...
	public void phoneUnique(HttpServletRequest request,
		...
		// email存在，返回false。
		if (!cmsUserMng.mobileNotExist(phone)) {
			ResponseUtils.renderJson(response, "false");
			return;
		}
		ResponseUtils.renderJson(response, "true");
	...
	private WebErrors validateSubmit(String username, String email,
			String loginPassword, String type, String captcha, CmsSite site,
			HttpServletRequest request, HttpServletResponse response) {
		MemberConfig mcfg = site.getConfig().getMemberConfig();
		...
		if (errors.ifNotType(type, MessageResolver.getMessage(request, "field.type"), 100)) {
			return errors;
		}
		// 保留字检查不通过，返回false。
