...
public class WeiXinAuthAct {
	...
	public String weixinAuth(HttpServletRequest request,
		...
		String redirect_uri="/member/weixin_auth.jspx";
		if(StringUtils.isNotBlank(site.getContextPath())){
		...
		try {
			codeUrl=getWeixinAuthCodeUrl()+"?appid="+config.getWeixinAppId()+"&redirect_uri="+ URLEncoder.encode(redirect_uri,"utf-8")
                    +"&response_type=code&scope=snsapi_userinfo&state=jeecms#wechat_redirect";
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		model.addAttribute("codeUrl", codeUrl);
		String ftl = FrontUtils.getTplPath(request, site.getSolutionPath(),
				TPLDIR_ALONE, MEMBER_WEIXIN_AUTH);
        return ftl;
	}
    ...
    //获取微信用户openid
    @RequestMapping(value = "/common/getOpenIdUrl.jspx", method = RequestMethod.GET)
    public String getWeixinOpenIdUrl(String rediretUrl,
                                HttpServletRequest request,
                                HttpServletResponse response, ModelMap model) {
        CmsSite site = CmsUtils.getSite(request);
        FrontUtils.frontData(request, model, site);
